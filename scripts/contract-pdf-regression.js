...
const contract_template_service_1 = require("../src/modules/api/documentos/contract-template.service");
const pdf_browser_service_1 = require("../src/modules/api/documentos/pdf-browser.service");
...
const normalize = (value) => String(value || '')
    ...
    .toLowerCase()
    .trim();
const buildLongClauses = (titlePrefix, profile = 'long', clauseCount = 24) => {
    const clauses = [];
    for (let i = 1; i <= clauseCount; i += 1) {
        const clauseBody = profile === 'medium'
...
async function run() {
    ...
    const pdfBrowserService = new pdf_browser_service_1.PdfBrowserService();
    const service = new contract_template_service_1.ContractTemplateService(pdfBrowserService);
    ...
    for (const scenario of scenarios) {
        ...
        await (0, promises_1.writeFile)(targetFile, pdf);
...
//# sourceMappingURL=contract-pdf-regression.js.map