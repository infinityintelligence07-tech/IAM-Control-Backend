...
@Injectable()
export class AdminOrLiderPuroGuard implements CanActivate {
        ...
        try {
            ...
            if (!usuario.funcao || !Array.isArray(usuario.funcao)) {
                throw new ForbiddenException(
                    'Acesso negado. Apenas administradores ou líderes podem aprovar cadastros de usuários.',
                );
            }

            if (usuario.funcao.includes(EFuncoes.ADMINISTRADOR)) {
                return true;
            }
            ...
            throw new ForbiddenException(
                'Acesso negado. Apenas administradores ou usuários com função Líder (sem funções de líder de evento ou masterclass) podem aprovar cadastros.',
            );
        } catch (error) {
            if (error instanceof ForbiddenException) {
                throw error;
            }
            throw new ForbiddenException('Erro ao verificar permissões para aprovação de usuários');
        }
    }
}
